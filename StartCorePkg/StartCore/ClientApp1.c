...
/** Wait for the Root task (Parent) to consume my data.
    We will wait "forever".

    @param[in,out]    Tcb     Pointer to the task control structure.
**/
VOID
WaitForRoot(IN TCB *Tcb)
{
...
ClientTask( IN VOID* parameter)
{
  ...
  do {
    ...
    Tcb->Ready = 1;             // Indicate that there is data in Result.
