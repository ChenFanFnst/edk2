  ...
  )
{
  ...
  Print(L"%08x      %s      %3s  %08x  %4d    %2d      %d\n\n",
      ...
      (UINT32)ProcessorContextBuffer->PackageNumber,
      (UINT32)ProcessorContextBuffer->NumberOfCores,
      (UINT32)ProcessorContextBuffer->NumberOfThreads);
...
)
{
  ...
  Print(L"Processor %d is the target processor, %d is the BSP.\n", (UINT32)Tcb->ProcNum, (UINT32)mUtilityData->BspId);
  ...
  )
{
  ...
  if (!Tcb->Blocked) {
    if (Status == EFI_SUCCESS) {
      ...
      Print(L"    Wait Failed Finished Status: %d.\n", Tcb->Finished);
    }
